...
- **Rectangle Tetris → "בונים קיר" (build the wall) — shipped, package 4 (2026-08-29).** The pure core + simulator (`tetris-wip` local branch, commit 7c5b245, not pushed) had shown that "orientation as asked, no rotation/sliding" gives a row-completing placement only ~23 % of the time (gate required 40 %) for every COLS ∈ {10,12,14} × ROWS ∈ 14–20 — the row-scoring mechanic itself couldn't work. Marat chose option B ("build the wall": rectangles stack, a full wall = celebration + fresh wall, coins only from facts, no row scoring) on 2026-08-29; `docs/WALL-DESIGN.md` is the shipped design, mode `"wall"`, `CONFIG.WALL`, `Wall` core section, `state.parkedSessions`/schema 3 for three concurrent modes. **Ships disabled by default** (parent settings toggle) — nothing changes for her until a parent turns it on.
- **Child playtest** of the new features on her iPad (spectators, names, 20-question rounds if enabled, album 2 progress, **and now "בונים קיר" once a parent enables it**) — the balloon start delay (0.6 s), the 8 s mastery threshold, and the wall's default duration/options are CONFIG numbers to tune from observation.
...
- **Build-the-wall shipped disabled (0.17.0, 2026-08-29).** Enable it in the parent view → "בונים קיר 🧱 — מצב נוסף" (duration slider, 4–6 options). Playtest with the child before leaving it on. Known cosmetic LOWs: at 320×568 the well is tiny (iPad is the target); reduced-motion column moves jump to the landing row early; the a×b label overflows a 1-wide piece. `tetris-wip` (local branch) is superseded and can be deleted.
