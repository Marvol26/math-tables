...
## 5. Out of scope
Mixing typed and falling in one session; falling mode affecting mastery; sounds beyond the existing blips; landscape-specific lane layouts beyond "lanes shrink to fit".

## 6. Tests (node)
Distractors: count = options−1, unique, ≠ p, all in 1…100, at least one same-table neighbour when it exists, deterministic under an injected rng; `start({mode:"falling"})` sets `active.mode`; `finish()` for falling: coins/ledger/session recorded with `mode:"falling"`, **facts untouched, carryover untouched, map untouched**; `Stats` excludes falling attempts; migrate defaults; validateImport accepts old sessions without `mode`.
