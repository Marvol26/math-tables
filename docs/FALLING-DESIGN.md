...
| F3 | A falling session = `settings.sessionSize` facts (10–20, parent slider, amended V2-DESIGN §3.3, 2026-08-29 — was a fixed 10) from the same `Selector.plan` (carryover/focus/review as usual) so it feels consistent. Falling still never touches `state.carryover` (I-F1); only the plan length changed. |
