...
- **Celebration**: when `finish()` reaches a new station, the summary gets a banner "הגעת לתחנה ×5! 🎉" with the station badge popping in, confetti, and the unlock sound; several stations in one session share one banner. When the gilded sticker for that station is already unlocked (see "Golden stickers" below), its emoji shows in the same banner.
- **Golden stickers (V2-DESIGN §3.4, added 2026-08-29)**: reaching the station at path *position* `k` (1…10, i.e. the k-th entry of `CONFIG.MAP_PATH`, not the table number) gilds sticker *k* of collection album 1 (`Economy.goldenStickers(state)`, purely derived — no new state field). Gilding is invisible until the sticker is separately unlocked by coins: a locked-but-gilded sticker never reveals itself anywhere (collection shelf, summary, or this map's station detail card).
