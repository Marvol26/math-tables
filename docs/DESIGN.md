...
**Runtime:** `queue`, `retryQueue`; wrong → 1.8 s answer display → push to retry; when queue empties, retry loop until empty. Progress = 10 dots; retries show 🔁.
**Journal (D12):** `state.active = {id, startedAt, settingsSnapshot, planned, queue, retryQueue, attempts, current:{key, asked, shownAt, interrupted}}` lives **inside the main state blob** — one object, one write, no second key (R2 #1, #6). It is saved (a) at session start with the full plan (so relaunch or exit can never re-roll the plan), (b) when a question is painted (`current`), (c) after every submit. On launch with `state.active` present: home shows "ממשיכים מאיפה שעצרנו" and resumes at `current`; a question resumed after a relaunch/lifecycle interruption is marked `interrupted` (base coins only, no challenge bar, neutral "ממשיכות בלי שעון" label — R2 #5). Exit (✕ → "יוצאות? נמשיך אחר כך מאותו מקום") only suspends; carried-over facts inside the plan therefore never disappear (R2 #2 / R1 #3).
...
**Timing (R1 #12):** `performance.now()` captured in a `requestAnimationFrame` after the equation is painted; stopped at submit; feedback time excluded. `visibilitychange` → hidden during a question, or a question resumed from `state.active.current` after relaunch, marks it `interrupted:true`: it earns base coins if correct, counts for accuracy, but is excluded from speed stats, `withinLimit`, and mastery-time; the challenge bar is hidden for that question and replaced by "ממשיכות בלי שעון" (it does not restart — R2 #5).
...
2. **Question**: equation `7 × 8 = [ ]` (LTR island), "שווה 3 🪙" badge, 10 progress dots, numpad (0–9, ⌫, ✓) when `pointer: coarse`, keyboard otherwise, "הצג מקלדת" toggle for hybrids. Challenge ON: soft draining bar under the card, no digits; at zero: "אפשר לאט 🙂", question stays open. Correct: green flash, coins fly to the balance pill, confetti. Wrong: soft orange shake, answer shown 1.8 s. ✕ / Escape → "יוצאות? נמשיך אחר כך מאותו מקום" (suspend only, D12).
