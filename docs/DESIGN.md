...
Per fact: `attempts`, `correct`, accuracy, median ms of the last 3 correct first attempts, lastSeen; mastery `new` / `mastered` (last 3 first attempts all correct AND their median ms ≤ `CONFIG.MASTERY_MS_THRESHOLD` AND none `interrupted`) / `learning`. Per session: firstTryCorrect, avg first-attempt ms (clamped 30 s, `interrupted` excluded), misses, retries count, coins, challengeOn+limit, abandoned flag, masteredAfter. Trends: accuracy, avg time, mastered count, coins; 10×10 heatmap; weakest 8; totals + daily streak.
