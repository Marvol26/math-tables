...
**Runtime:** `queue`, `retryQueue`; wrong → the dot-array helper stays open until the child taps "הבנתי" or presses Enter (no auto-advance timer since 2026-08-27 — see §9's Question screen for the mechanism) → push to retry; when queue empties, retry loop until empty. Progress = 10 dots; retries show 🔁.
